    ...
    switch (t) {
      case 0: { // POLYGON / LINE
        ...
        if (pts && pts.length >= 2) {
          ...
          // If the user manually drew back to the start, the path is already geometrically closed.
          // Forcing builder.close() here adds a degenerate zero-length segment that breaks CanvasKit's stroker.
          if (shape.close === true || shape.closed === true) {
            builder.close();
          }
        }
        break;
      ...
      case 3: { // ELLIPSE
        const cx = shape.x ?? 0;
        const cy = shape.y ?? 0;
        const rx = shape.width ?? shape.rx;
        const ry = shape.height ?? shape.ry;
